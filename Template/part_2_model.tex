...
We can use the following codes to describe a part of calculation.\footnote{This section demonstrates the aesthetic formatting environment for mathematical expressions} 

\textbf{Examples of Theorem Environment Usage:}
\begin{Definition}
    A covariance matrix is a symmetric matrix that describes the covariance relationships between multiple random variables. For $n$ random variables $X_1, X_2, \ldots, X_n$, the covariance matrix $\Sigma$ is defined as:
    \[
    \Sigma_{ij} = \text{Cov}(X_i, X_j)
    \]
\end{Definition}

\begin{Theorem}[Matrix Eigenvalue Decomposition]
    For any real symmetric matrix $A$, there exists an orthogonal matrix $Q$ and a diagonal matrix $\Lambda$ such that:
    \[
    A = Q\Lambda Q^T
    \]
    where the diagonal elements of $\Lambda$ are the eigenvalues of $A$, and the column vectors of $Q$ are the eigenvectors of $A$.
\end{Theorem}

\begin{Example}[Covariance Matrix Calculation Example]
    Consider two random variables $X$ and $Y$ with sample data:
    \[
...
\begin{figure}[H]
    \centering   %使用subfigure环境排版三张图也是可以的,但是需要调整图宽度 
    \begin{subfigure}[t]{0.31\linewidth}
        \centering
        \includegraphics[width=\linewidth]{catsquare.jpeg}
...
\begin{figure}[H] %[H]让图片在文章中的位置就是这段代码的位置
	\centering
		\begin{minipage}[t]{0.32\textwidth} %并排排列图片使用\minipage{}环境,而子图排列使用\subfigure{}环境
			\centering
